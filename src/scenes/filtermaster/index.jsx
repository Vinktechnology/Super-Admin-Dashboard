...
import { getAllFilterThunk, updateFilterStatusThunk } from "../../store/slices/filter/filter.slice.js";
...
const FilterMaster = () => {
  ...
  const handleView = (id) => {
    navigate(`/dashboard/filter/view/${id}`);
  };

  const handleEdit = (id) => {
    navigate(`/dashboard/filter/addfilter/${id}`);
  };
  ...
  const fncHandleDialog = (isConfirmed) => {
    if (isConfirmed) {
      dispatch(updateFilterStatusThunk({ ...toggleDATA }));
    } else {
      setSwitchChecked(prevState => ({
        ...prevState,
  ...
  return (
    <Box p={2}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header title="Filter" subtitle="Manage Filter" />
        ...
        <Box>
          <Button
            ...
            onClick={() => navigate("/dashboard/filter/addfilter")}
          >
            <AddIcon sx={{ mr: "10px" }} />
            Add Filter
          </Button>
